# Circle Live Folders - Specification

## Overview

Live Folders are dynamic, auto-updating folders in Circle.  
...
REST-based Live Folders allow Circle to fetch JSON data from an HTTP(S) endpoint and map it into folder items.  
Each REST Live Folder must provide a **schema-compliant response** that Circle can parse into items.

- **Remote APIs (https://, http://)**: Flexible schema (mapping via config).
- **Localhost APIs (http://127.0.0.1, http://localhost)**: Must strictly follow Circle’s **Local REST Schema** for security and consistency.
...
- CORS headers are ignored (Circle fetches internally).
...
These schemas would be stored inside a marketplace on Circle's web platform, allowing users to easily discover and integrate new REST API Live Folders into their workspace.
